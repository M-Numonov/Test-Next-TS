...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/fileds/filedsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureFiledsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSampleFileds = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    notify: filedsNotify,
    refetch,
  } = useAppSelector((state) => state.fileds);
  ...
  useEffect(() => {
    if (filedsNotify.showNotification) {
      notify(filedsNotify.typeNotification, filedsNotify.textNotification);
    }
  }, [filedsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/fileds/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/fileds/fileds-view/?id=${id}`);
  };
...
export default TableSampleFileds;
